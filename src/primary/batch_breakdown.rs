...
use crate::executor::api::{
    ...
    ProxyToProxyMessage, TransactionWithTimestamp,
};
...
impl MeasuredMessage for usize {
    fn measurement_digest(&self) -> Option<TransactionDigest> {
        None
    }
}

impl MeasuredMessage for i32 {
    fn measurement_digest(&self) -> Option<TransactionDigest> {
        None
    }
}
...
impl Default for BatchBreakdownState {
    fn default() -> Self {
        Self {
            digests: Vec::new(),
            tx_count: 0,
            network_rx_deser: StageTiming::default(),
            batch_scheduling: StageTiming::default(),
            version_assignment: StageTiming::default(),
            dispatch_forwarding: StageTiming::default(),
            network_tx_serialize: StageTiming::default(),
            // The post-consensus branch has no separate batch scheduling stage.
            scheduling_recorded: true,
            version_assignment_recorded: false,
        }
    }
}
...
impl BatchBreakdownCollector {
    pub(crate) fn register_batch<M>(&self, messages: &[M]) -> Option<u64>
    where
        M: MeasuredMessage,
    {
    ...
    pub(crate) fn batch_id_for_messages<M>(&self, messages: &[M]) -> Option<u64>
    where
        M: MeasuredMessage,
    {
...
mod tests {
    ...
    fn batch_summary_reports_wall_clock_without_summing_overlaps() {
        ...
        assert_ms_eq(summary.batch_scheduling_ms, 0.0);
    ...
    fn summary_format_uses_explicit_sum_avg_and_wall_labels() {
        ...
        assert!(line.contains("batch_scheduling_ms=0.000"));
